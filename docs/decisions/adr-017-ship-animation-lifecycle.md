...
🐛 **Found 2026-07-24:** `game-over` *is* a session boundary and does unmount `MapView`/`CityView` (correctly, per this ADR) — but their `destroy()` methods had a latent double-destroy bug: `CityScene.destroy()` called `sceneManager.destroy()` (which explicitly destroyed each registered scene's PixiJS container) immediately before calling `app.destroy(true, { children: true })`, which recursively destroys the same containers again. This corrupted PixiJS's shared canvas-text texture pool ("Cannot read properties of undefined (reading 'push')" thrown from deep inside PixiJS on the *next* scene teardown anywhere in the app), producing a fully blank screen — the exception aborted Svelte's in-flight screen transition mid-swap. Rarely hit before this ADR's own game-over path existed and while sessions were fixed-length (40 turns, few ever ending in bankruptcy); far more exposed once `family-succession.md` (ADR-022) made death the primary way sessions end. Fixed by making the per-scene `destroy()` callback in `city-scene.ts` a no-op (the app-level destroy already covers it) and wrapping both `MapScene.destroy()`/`CityScene.destroy()`'s `app.destroy()` calls in try/catch as defense-in-depth, since the scene is being discarded either way and an uncaught throw here has no recovery value.
