...
18. ✅ **v1.2 item 1: Supply/Demand moved off the trading table onto Town Hall** (`design/market-formula.md`, `design/city-view.md`) — trading table now `Good | Price | Stock | Buy | Sell`; Town Hall gained a per-good, read-only Supply/Demand table for the selected city. New shared `src/ui/TradeTable.svelte` replaces 4 previously hand-duplicated table copies across City-view/List-view. The rest of v1.2 (turn counter, map icons, qty-1 total, Sell-all, test-policy decision, `/check-conventions` enforcement) is still open in `design/roadmap-next-versions.md`.
