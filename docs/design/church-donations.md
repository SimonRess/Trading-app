...
### Revised (2026-07-31): rate-raise/UI drift bug fixed

The "Throttled progress" and "UI" sections above (and the "Implementation Status" verification note below) describe the mechanic as it worked *before* the 2026-07-25 ×10 rate raise (Open Questions, below) — "50 Mark = 1%", "1% per turn" — left as originally written rather than rewritten, since it's an accurate record of what was actually built and verified at the time. But `App.svelte`'s own display code was never updated to match the ×10 raise: its "~N more turns" estimate and the pledged-progress bar's width both kept a hardcoded `/ 50` divisor after `church-system.ts`'s actual constant became `DONATION_COST_PER_PERCENT = 500`. A player who pledged 1,000 Mark saw "~20 more turns" (computed as `1000 / 50`) instead of the correct ~2 (`1000 / 500`), and the pledged-overlay bar rendered 10× too wide.

Fixed by exporting `DONATION_COST_PER_PERCENT`, `REPUTATION_COST_PER_POINT`, `PROGRESS_CAP_PER_TURN`, and a derived `MAX_MARK_PER_TURN` from `church-system.ts` and having `App.svelte` compute its estimate and bar width from those instead of separately hardcoded numbers — the exact fix requested when this was reported: define the values once, reuse them everywhere, so a future rate change can't silently desync the UI from the game logic again the way this one did for six days.
