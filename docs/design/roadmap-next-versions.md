...
9. **Ship convoys** (redefined 2026-08-02, replacing the earlier "saved/repeating trade routes" scope below; fully specified 2026-08-02 in `design/ship-convoys.md`) — group several ships into a convoy so the fleet overview shows one entry per convoy instead of one per ship, cleaning up the panel once a player has more than 2-3 ships. Confirmed mechanics (see the design doc for the full spec):
   ...
   - **ADR-023** (Proposed) covers the data model and ship-addressing decision — `FleetState.convoys: Convoy[]`, no `convoyId` on `Ship`, new convoy-addressed action variants alongside the existing ship-addressed ones. Still needs a **UI-design pass** before implementation starts — both design docs cover game logic only. Three cosmetic/edge-case questions remain open in `ship-convoys.md` (posture persistence on leaving a convoy, whether a convoyed ship must be excluded before auctioning, convoy default naming).
