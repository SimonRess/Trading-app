...
9. **Ship convoys** (redefined 2026-08-02, replacing the earlier "saved/repeating trade routes" scope below; fully specified 2026-08-02 in `design/ship-convoys.md`) — group several ships into a convoy so the fleet overview shows one entry per convoy instead of one per ship, cleaning up the panel once a player has more than 2-3 ships. Confirmed mechanics (see the design doc for the full spec):
   - Drill into a convoy to see its individual ships; exclude a ship (only while in port).
   - Repair, and buying/selling weapons (cannons) and crew, stay **per-ship** actions.
   - Buying/selling **goods** is **convoy-wide**: one `resolveTradeStepped` call for the total quantity, then distributed across ships via a swappable strategy (proportional by default).
   - **Convoy ships travel and fight as one unit**: `Set Destination` is convoy-wide, travel time = the slowest member's; a convoy is pirate-targeted as a whole, fights with summed combat power and a convoy-wide posture, and combat's durability/cargo loss applies independently to every member ship (a defeat can sink a weak member while stronger ones survive).
   - Data model: `FleetState.convoys: Convoy[]` (additive, `shipIds: string[]` membership — no `convoyId` on `Ship` itself).
   - **ADR-023** (Proposed) covers the data model and ship-addressing decision — `FleetState.convoys: Convoy[]`, no `convoyId` on `Ship`, new convoy-addressed action variants alongside the existing ship-addressed ones. **UI design is now specified too** (`ship-convoys.md` "UI Design"): grouped/collapsible fleet-panel rows, a `selectedConvoyId`/`activeConvoy` selection pair parallel to the existing ship selection, `TradeTable.svelte` extended (not duplicated) with an optional convoy-cargo prop pair, and a convoy-wide posture selector replacing the per-ship one while grouped. Spec is complete enough to start implementation; two small open questions remain (posture persistence after leaving a convoy, whether exclusion is required before auctioning a convoyed ship).
