...
9. **Ship convoys** (redefined 2026-08-02, replacing the earlier "saved/repeating trade routes" scope below) — group several ships into a convoy so the fleet overview shows one entry per convoy instead of one per ship, cleaning up the panel once a player has more than 2-3 ships. Player-specified mechanics:
   - Drill into a convoy to see its individual ships; can exclude (remove) a ship from the convoy.
   - Repair, and buying/selling weapons (cannons) and crew, stay **per-ship** actions — reached by drilling into the convoy, not convoy-wide.
   - Buying/selling **goods** works at the **convoy level** — a single buy/sell order distributes the goods across the convoy's ships automatically.
   - Larger than 6-8, and now larger than the "saved trade routes" idea it replaces: touches fleet data model, the Harbor/Trading Post UI, and market pricing. Needs its own design doc before starting — open questions for that doc, not resolved here:
     - **Do convoy ships travel together?** Grouping ships to sail as one unit is the natural "why" of a convoy, but the request as given only specifies goods trading and the overview UI, not travel/destination-setting. Needs an explicit decision: does "Set Destination" become convoy-wide too, or does convoy grouping only affect goods and the overview panel while each ship still sails independently?
     - **Goods distribution algorithm** — when a convoy-level buy order lands, how is the purchased quantity split across member ships' cargo holds? (e.g. fill-fullest-first, round-robin, proportional to remaining capacity.) Needs a concrete formula.
     - **Bulk-pricing interaction** — `resolveTradeStepped` (`market-formula.md`) moves the market price as a single order fills. A convoy-wide buy of N units should almost certainly resolve as one stepped trade against the market (not N/shipCount separate trades, which would price differently), then distribute the resulting goods across ships — needs to be specified precisely.
     - **Selling** — does a convoy-level sell order pull from every member ship's hold of that good, summed, in one bulk trade the same way buying does?
     - Data model: likely a new `Convoy` concept in `FleetState` (e.g. `Ship.convoyId?: string` or a `convoys: Convoy[]` list) — needs an ADR-level look since it changes how ships are addressed throughout `fleet-system.ts` and the UI's `shipyardShips`/`activeShip` patterns.
