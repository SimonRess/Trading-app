...
Fixed by keeping `<MapView>` permanently mounted inside the port screen and toggling only its container's CSS visibility (`class:hidden`, `display: none`) based on `screen`. `MapScene.update()` still runs on every state change regardless of visibility, so the map is current the instant it's shown.

**Follow-up fix — turn-summary also tore it down:** the persistent-mount scope was originally "narrower" — switching to the turn-summary screen after End Turn was a separate `{:else if screen === 'turn-summary'}` branch, which still unmounted `<MapView>` (destroying its `MapScene`/PixiJS `Application`) on *every single turn*, since End Turn always routes through that screen. This silently broke the ship-movement animation (see "Ship Movement Animation" below) for its actual use case: a fresh `MapScene` always starts markers already at their final position, so turn-to-turn ship movement never had a "before" position to glide from — the animation only ever worked for the Map↔Port toggle within a turn, which doesn't move ships. Fixed by turning turn-summary into an overlay (`.turn-summary-overlay`) rendered on top of the still-mounted port/map view instead of a separate screen branch, so `<MapView>` now only unmounts on genuine session boundaries (new game, game over — losing specifically; winning is no longer a session boundary at all, see `political-rank.md`, and reuses this same overlay with a "Continue Playing" option instead of routing to `game-over`).
...
- **Routes:** straight lines between connected cities (per `ROUTES`), tinted by current combined storm+pirate danger (see "Route Danger Colouring" below); a route with a player ship currently on it is drawn thicker and blended toward gold, but the danger tint still shows through (2026-07-25 fix — it used to fully override to plain gold, hiding the exact case a player most needs to see: a storm hitting a route their ship is actually on)
...
**Pausing while hidden:** the tween runs on wall-clock time (`performance.now()`), independent of whether the map is actually on screen. Without accounting for this, a ship that departs while the player is on the Port screen (map hidden) would finish its glide invisibly in the background, and by the time they switch to the map it'd already be snapped to its final position with nothing to see. `MapView.svelte` calls `MapScene.setVisible(visible)` whenever its `visible` prop changes; `tickShipAnimations` skips repositioning entirely while hidden (freezing markers at wherever they were), and `setVisible(true)` recomputes each marker's `animStart` from how far it had actually progressed *before* going hidden (0 if its target changed after already going hidden), so the glide resumes exactly where it visually left off rather than wherever raw elapsed wall-clock time would place it.
