...
**Bug fixed (2026-07-25):** the button's own total-cost text initially stayed frozen after repeated trades — Svelte's per-node dirty-check for a `{@const}`-derived value only tracks identifiers textually present in that specific `{@const}` declaration, and `state`/`selectedCityId` were only read inside the called preview function, invisible to that analysis (the same class of bug as an earlier `activeShip` reactivity fix in this file). The `title`/`disabled` attributes happened to reference `state`/`cityMarket` directly elsewhere and updated correctly, which is what made the frozen button text easy to miss. Fixed by making `buyPreview`/`sellPreview` take `state`/`cityId` as explicit parameters, so every call site references them directly.
